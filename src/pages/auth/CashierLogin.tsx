...
import { Mail, Lock, Users, AlertCircle, Sparkles, ShieldAlert } from 'lucide-react';
...
export default function CashierLogin({ onLoginSuccess }: CashierLoginProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!email || !cashierUsername || !pin) return;
    setErrorMsg(null);
    setLoading(true);
    try {
      const user = await authService.signInCashier(email, cashierUsername, pin);
      onLoginSuccess(user);
    } catch (err: any) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 text-left">
      ...
      <div>
        ...
        <div className="relative">
          <input
            ...
            className="w-full bg-slate-950/45 border border-slate-700/60 rounded-xl pl-11 pr-4 py-3 text-xs text-white placeholder-slate-550 focus:outline-none focus:ring-2 focus:ring-indigo-500/25 focus:border-indigo-500 transition-all font-mono font-bold"
          />
      ...
      <div>
        ...
        <div className="relative">
          <input
            ...
            className="w-full bg-slate-950/45 border border-slate-700/60 rounded-xl pl-11 pr-4 py-3 text-xs text-white placeholder-slate-550 focus:outline-none focus:ring-2 focus:ring-indigo-500/25 focus:border-indigo-500 transition-all font-mono font-bold tracking-widest"
          />
      ...
      <div className="bg-indigo-950/40 border border-indigo-900/30 p-3.5 rounded-2xl text-[10px] text-indigo-300 flex items-start gap-2.5 font-semibold">
        ...
        <span>
          **Cashier Terminal**: Enter the registered email address of your Store Administrator, followed by your custom secure cashier username and secure numeric PIN code.
        </span>
      </div>
      ...
      >
        {loading
          ...
          : <><Sparkles className="h-4 w-4" /><span>Open POS Terminal</span></>
        }
      </button>
    </form>
  );
